import { test, describe, it, expect, beforeAll, beforeEach, afterAll, afterEach } from '@enxoval/testing';
import { TestDataSource } from './helpers/data-source';

test.mock('../../src/db/data-source', () => ({ AppDataSource: TestDataSource }));
test.mock('@enxoval/auth', () => ({ setupAuth: test.fn() }));
test.mock('@enxoval/messaging', () => ({
  publish: test.fn(),
  connect: test.fn(),
  disconnect: test.fn(),
}));

import { buildApp } from '../../src/app';
import { publish } from '@enxoval/messaging';
import { AppDataSource } from '../../src/db/data-source';
import { JourneyDbWire } from '../../src/db/wire/journey';
import { CurriculumGeneratedDbWire } from '../../src/db/wire/curriculum-generated';
import { ContentDispatchedDbWire } from '../../src/db/wire/content-dispatched';
import { curriculumGenerated } from '../../src/controllers/curriculum-generated';

const journeyId = '11111111-1111-1111-1111-111111111111';
const cgId = 'ffffffff-ffff-ffff-ffff-ffffffffffff';

beforeAll(async () => {
  await TestDataSource.initialize();
  buildApp();
});

afterAll(async () => {
  await TestDataSource.destroy();
});

beforeEach(() => {
  test.clearAll();
});

afterEach(async () => {
  await AppDataSource.getRepository(ContentDispatchedDbWire).clear();
  await AppDataSource.getRepository(CurriculumGeneratedDbWire).clear();
  await AppDataSource.getRepository(JourneyDbWire).clear();
});

async function seed() {
  await AppDataSource.getRepository(JourneyDbWire).save(
    AppDataSource.getRepository(JourneyDbWire).create({
      id: journeyId,
      student_id: '22222222-2222-2222-2222-222222222222',
      current_step: 'CURRICULUM_GENERATED',
      status: 'active',
    }),
  );
  await AppDataSource.getRepository(CurriculumGeneratedDbWire).save(
    AppDataSource.getRepository(CurriculumGeneratedDbWire).create({ id: cgId, journey_id: journeyId }),
  );
}

describe('curriculumGenerated consumer', () => {
  it('inserts ContentDispatched with id = curriculumGenerated.id', async () => {
    await seed();
    await curriculumGenerated({ eventId: cgId, journeyId });

    const records = await AppDataSource.getRepository(ContentDispatchedDbWire).find();
    expect(records).toHaveLength(1);
    expect(records[0].id).toBe(cgId);
    expect(records[0].journey_id).toBe(journeyId);
  });

  it('updates journey current_step to CONTENT_DISPATCHED', async () => {
    await seed();
    await curriculumGenerated({ eventId: cgId, journeyId });

    const journey = await AppDataSource.getRepository(JourneyDbWire).findOneByOrFail({ id: journeyId });
    expect(journey.current_step).toBe('CONTENT_DISPATCHED');
  });

  it('publishes contentDispatched with correct payload', async () => {
    await seed();
    await curriculumGenerated({ eventId: cgId, journeyId });

    expect(publish).toHaveBeenCalledOnce();
    const [topic, payload] = (publish as ReturnType<typeof test.fn>).mock.calls[0];
    expect(topic).toBe('contentDispatched');
    expect(payload.journeyId).toBe(journeyId);
    expect(payload.eventId).toBe(cgId);
  });

  it('is idempotent — second call re-publishes without inserting duplicate', async () => {
    await seed();
    await curriculumGenerated({ eventId: cgId, journeyId });
    await curriculumGenerated({ eventId: cgId, journeyId });

    const records = await AppDataSource.getRepository(ContentDispatchedDbWire).find();
    expect(records).toHaveLength(1);
    expect(publish).toHaveBeenCalledTimes(2);
  });
});
